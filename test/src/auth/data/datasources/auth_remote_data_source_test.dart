...
class MockVerifiedUserCredential extends Mock implements UserCredential {
  MockVerifiedUserCredential([User? user]) : _user = user ?? MockUser();
  User? _user;
  ...
  group('signIn', () {
    ...
    test(
      ...
      () async {
        ...
        final call = dataSource.signIn;
        final result = await call(email: tEmail, password: tPassword).then(
          (value) => value.uid,
        );
  ...
  group('signUp', () {
    test(
      ...
      () async {
        ...
        when(() => userCredential.user?.sendEmailVerification()).thenAnswer(
